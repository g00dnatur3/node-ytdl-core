...
const torRequest = (url, opts={}) => {
  ...
  return new Promise((resolve, reject) => {
    const req = tr.request(reqOptions, (err, res, body) => {
      if (err) {
        reject(err)
      } else {
...
const doGet = async (url, opts={}, retryCount=10) => {
  const {req, res, body} = await torRequest(url, opts)
  if (body.indexOf('unusual traffic from your computer network') !== -1 || 
      body.indexOf('We have been receiving a large volume of requests from your network') !== -1) {
    console.log('WARN: unusual traffic from your computer network, retryCount', retryCount)
    if (retryCount > 0) {
      await newTorSession()
      retryCount--
      return await doGet(url, opts, retryCount)
    }
    else {
      throw Error('unusual traffic from your computer network')
    }
  }
  else {
    return {req, res, body}
...
exports.getBasicInfo = async(id, options) => {
  ...
  console.log('INFO AFTER PIPELINE: ', info)
...
const getWatchHTMLPageBody = async (id, options) => {
  console.log('GOT HERE - getHTMLWatchPageBody - enter')
  //miniget(url, options.requestOptions).text()
  const url = getWatchHTMLURL(id, options);
  console.log()
  console.log('url', url)
  console.log('options.requestOptions', options.requestOptions)
  console.log()
  const {body} = await doGet(url, options.requestOptions)
  console.log('GOT HERE - getHTMLWatchPageBody - exit')
  return exports.watchPageCache.getOrSet(url, () => body)

};

const EMBED_URL = 'https://www.youtube.com/embed/';
const getEmbedPageBody = async (id, options) => {
  console.log('GOT HERE - getEmbedPageBody - enter')
  const embedUrl = `${EMBED_URL + id}?hl=${options.lang || 'en'}`;
  const {body} = await doGet(embedUrl, options.requestOptions)
  //return utils.exposedMiniget(embedUrl, options).text();
  console.log('GOT HERE - getEmbedPageBody - exit')
  return body
};
...
const findPlayerResponse = (source, info) => {
  ...
  console.log('findPlayerResponse.source:', source)
  console.log('findPlayerResponse.info:', info)
...
const getWatchJSONPage = async(id, options) => {
  ...
  console.log('getWatchJSONPage - jsonUrl:', jsonUrl)
  let body = await utils.exposedMiniget(jsonUrl, options, reqOptions).text();
  console.log('getWatchJSONPage - body:', body)

  if (body.includes('{"reload":"now"}')) {
    await setIdentityToken('browser', false);
    body = await utils.exposedMiniget(jsonUrl, options, reqOptions).text();
  }
  
  // if (body.trim().length < 50) {
  //   const data = await doGet(jsonUrl, reqOptions)
  //   body = data.body
  //   console.log('getWatchJSONPage - body.tr:', body)
  // }
...
const getVideoInfoPage = async(id, options) => {
  console.log('GOT HERE - getVideoInfoPage - enter')
  ...
  const {body} = await doGet(url.toString(), options.requestOptions)
  ...
  console.log('GOT HERE - getVideoInfoPage - exit')
  return info;
};
...
const getM3U8 = async(url, options) => {
  console.log('GOT HERE - getM3U8 - enter')
  ...
  body
    ...
    console.log('GOT HERE - getM3U8 - exit')
  return formats;
};
