...
const doGet = async (url, opts={}, retryCount=10) => {
  ...
  if (body.indexOf('unusual traffic from your computer network') !== -1 || 
      body.indexOf('We have been receiving a large volume of requests from your network') !== -1 || body.trim() === `)]}'`) {
...
const getWatchJSONPage = async(id, options) => {
  ...
  const {body} = await doGet(jsonUrl, {...options, ...reqOptions})
  console.log('getWatchJSONPage - body:', body)

  if (body.trim().length < 10) {
    await newTorSession()
    return await getWatchJSONPage(id, options)
  }
